...
+ Headers With Ids (Levels 1-6)
  * Formatting sequence: `#` repeated n times followed by `[<id>]` or `h<n>[<id>]:` as the first non-whitespace characters in a line where n is a number 1-6 inclusive and corresponds to the desired level header and id the the desired identifier
  * Needs closing sequence: No
  * Resulting HTML Tag: `<h1>`, `<h2>`, `<h3>`, `<h4>`, `<h5>`, or `<h6>`
  * Example: `##[some_id] Header Two` or `h2[some_id]:Header Two`
  * Notes: To prevent id collisions, all ids generated by SHML are prefixed with `h<n>:` (So `h1[foo]: Hello` actually has an id of `h1:foo` instead of just `foo`).
