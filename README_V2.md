...
Version 2 of SHML uses PCRE instead of the built-in Regex engine for some passes. Because of this, it is only compatible with browsers that support WASM. (Note that it can still be used on the backend to generate files since modern Node.js supports WASM). If using PCRE dependant features, make sure to include the WASM file along with the JavaScript module. If the WASM filter is not found, SHML will continue working with a subset of it's features.
